...
// 每次刚进入网站, 会执行 index.js 在刚调用的时候, 先从本地存储中, 把购物车的数据读出来, 放到 store 中
let car = JSON.parse(localStorage.getItem('car') || '[]')

const store = new Vuex.Store({
  state: {  // this.$store.state.***
    car: car // 将购物车中的商品的数据，用一个数组存储，在car数组中，存储一些商品的对象，可以暂时将这个商品对象，设计成这样 
    // { id: 商品id, count: 要购买的数量, price: 商品单价, selected: true }
  },
  mutations: {  // this.$store.commit('方法名', '按需传递唯一的参数')
    addToCar(state, goodsinfo) {
      // 点击加入购物车，把商品信息保存到 store 中的 car 数组上
      ...
      // 如果最终循环完毕, 得到的 flag 还是 false, 则把商品数组直接 push 到 购物车中
      if (!flag) {
